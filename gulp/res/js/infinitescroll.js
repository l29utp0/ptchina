(() => {
    ...
    function extractPostData(postElement) {
        const postData = {
            ...
            files: Array.from(postElement.querySelectorAll('.post-file')).map(f => ({
                hash: f.dataset.hash,
        ...
        postData.quotes = Array.from(quoteLinks).map(link => {
            const quoteMatch = link.getAttribute('href').match(/\/thread\/(\d+)\.html#(\d+)$/);
            if (quoteMatch) {
                return {
    ...
    async function loadMore() {
        ...
        try {
            ...
            if (newThreads.length > 0) {
                const threadsContainer = document.querySelector('#threads-container .thread-form');
                const bottomPages = threadsContainer.querySelector('.pages.bottom-pages');
