...
const createBoard = require(__dirname+'/../../models/forms/create.js')
    , { Accounts, Boards } = require(__dirname+'/../../db/') // Add Accounts import
...
module.exports = {
    ...
    controller: async (req, res, next) => {
        ...
        const errors = await checkSchema([
            ...
            // Modified check for board ownership limit
            { result: async () => {
                // If user has ROOT permission, bypass the board limit check
                if (res.locals.permissions.get(Permissions.ROOT)) {
                    return true;
                }
                // Otherwise check if they already own a board
                const account = await Accounts.findOne(req.session.user);
                return account.ownedBoards.length === 0;
            }, expected: true, error: __('You can only own one board.') },
