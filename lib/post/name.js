...
const { getInsecureTrip, getSecureTrip } = require(__dirname+'/tripcode.js')
	...
	, staffLevels = ['l29utp0', 'Sysop', 'Admin', 'Mod'];
...
module.exports = async (inputName, permissions, boardSettings, boardOwner, boardStaff, username, __) => {
	...
	if ((permissions.get(Permissions.MANAGE_BOARD_GENERAL) || !forceAnon) && inputName && inputName.length > 0) {
		...
		if (matches && matches.groups) {
			...
			if (groups.capcode) {
				...
				if (permissions.get(Permissions.DONOR) && capcodeInput === 'ouro') {
					donor = true;
				} else if (permissions.get(Permissions.MANAGE_BOARD_GENERAL)) {
	...
	return { name, tripcode, capcode, donor };
