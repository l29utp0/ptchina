...
const updateMarkdownPerms = () => {
	replacements = [
		...
		{ permission: Permissions.USE_MARKDOWN_DETECTED, regex: detectedRegex, cb: (permissions, match, detected) => `<span class='big'>${detected}</span>` },
		{ permission: Permissions.USE_MARKDOWN_IMAGE, regex: imageRegex, cb: (permissions, match, altText, imageSrc) => `<img class='embimg' title='${altText}' src='${imageSrc}' />` },
