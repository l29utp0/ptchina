...
const countries = require('i18n-iso-countries')
	...
	, anonymizerCountryCodes = ['TOR', 'VPN', 'T1']
...
module.exports = {
	...
	getAnonymizerCode: (req) => {
		if (req) {
			const lvl = Number(req.headers['x-sus-level']);
			if (lvl > 0) {
				return 'VPN';
			}
		}
	...
	isAnonymizer: (code, req) => {
		const susCode = module.exports.getAnonymizerCode(req);
		if (susCode) {
			return true;
		}
